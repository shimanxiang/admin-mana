<template>
  <div class="userlist tableData">
         ...
         width="100px">
          <template slot-scope="scope">
            <el-button
              size="mini"
              type="text"
              @click="gotoOrder(scope.row.nickName)">订单</el-button>
            <el-button
              size="mini"
              type="text"
              @click="showCoupon(scope.row.id, scope.row.nickName)">优惠券</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
    ...
    <couponUser :userId="userId" :couponListAll="couponListAll" :userName="userName" v-show="isCoupon" v-on:cancel="showUserList"></couponUser>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    gotoOrder (name) {
      this.$router.push({ path: '/orderlist', query: { name: encodeURIComponent(name) }})
    },
    distributionCoupon () {
      if (this.loading) return false
      this.loading = true
