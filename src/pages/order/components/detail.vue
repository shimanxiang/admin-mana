  ...
  <div class="order-confirm">
    ...
    <el-card>
        <div class="order-info">
            ...
            <div>优惠金额：{{orderInfo.couponDenomination || '未使用任何优惠'}}</div>
            <div>总费用(不包含配送费 打包费)：{{orderInfo.totalFee}}</div>
            <div>订单实际总价(商品价+打包费+配送费-优惠券金额)：{{orderInfo.orderFee}}</div>
            <div>使用积分数：{{orderInfo.integral}}</div>
            <div>送货方式：{{orderInfo.deleveryTypeDesc}}</div>
            <div>配送时间：{{orderInfo.userDeliveryTime}}</div>
            <div v-if="orderInfo.address">收货人姓名：{{orderInfo.address.name}}</div>
            <div v-if="orderInfo.address">收货人电话：{{orderInfo.address.telephone}}</div>
            <div v-if="orderInfo.address">收货地址：{{orderInfo.address.code1}}{{orderInfo.address.code2}}{{orderInfo.address.code3}}{{orderInfo.address.addressDetails}}</div>
        </div>
        ...
        <div v-if="orderInfo.user" class="order-info">
            ...
            <div>头像：<el-avatar size="medium" :src="orderInfo.user.headImg"></el-avatar></div>
