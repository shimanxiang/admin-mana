  ...
  <div class="coupon-form">
    ...
    >
      ...
      <el-form-item label="最低限制金额" v-show="couponForm.type == '1'">
        <el-input
          v-model="couponForm.limitPrice"
          onkeyup="this.value= this.value.match(/\d+(\.\d{0,2})?/) ? this.value.match(/\d+(\.\d{0,2})?/)[0] : ''"
        ></el-input>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" @click="submitForm('couponForm')">{{
          btnTxt
        }}</el-button>
  ...
  methods: {
    ...
    addCouponTemplate() {
      ...
      addCouponTemplate({
        ...
        limitPrice: this.couponForm.limitPrice
          ? parseFloat(this.couponForm.limitPrice)
          : "",
