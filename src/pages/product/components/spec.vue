  ...
  <div class="product-spec">
    ...
    >
      ...
      <el-form-item label="原价" prop="originalPrice">
        <el-input
          v-model="specForm.originalPrice"
          onkeyup="this.value= this.value.match(/\d+(\.\d{0,2})?/) ? this.value.match(/\d+(\.\d{0,2})?/)[0] : ''"
        ></el-input>
      </el-form-item>
      <el-form-item label="现价" prop="presentPrice">
        <el-input
          v-model="specForm.presentPrice"
          onkeyup="this.value= this.value.match(/\d+(\.\d{0,2})?/) ? this.value.match(/\d+(\.\d{0,2})?/)[0] : ''"
        ></el-input>
      </el-form-item>
      <el-form-item class="spec-btn">
        <el-button type="primary" @click="submitForm('specForm')"
          >新增</el-button
        >
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          ...
          <el-button
            type="text"
            size="small"
            @click="clickDeleteBtn(scope.$index, scope.row.id)"
            >删除</el-button
          >
        </template>
      </el-table-column>
